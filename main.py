...
color_config_yaml = ["standard_color = Color.white","lib_changed_color = Color.yellow"]
...
command_list = ["ls","cd","pwd","mkdir","grep","exit","head","less","mv","sudo","tail","clear","dos_fdisk","dos_type","dos_edit","print","man"]
...
def draw_screen():
    ...
    typed_line = f"{username}@{hostname}{current_directory}:{command}"
    
 
    if len(typed_line) > max_line_length:
        typed_line = typed_line[:max_line_length]  
    ...
    if flash_cursor_bool:
        ...
        cursor_position = min(len(typed_line), max_line_length) * 8 
        cursor.set_position(cursor_position, get_height() - 22)
...
def input_handler(e):
    ...
    if e.key not in ["ArrowLeft", "ArrowRight", "ArrowDown", "ArrowUp", "Control", "Alt", "Enter", "CapsLock", "Tab", "Shift","Backspace", "Meta","Escape","F1","F2","F3","F4","F5","F6","F7","F8","F9","F10","F11","F12","Insert","End","NumLock","Delete"]:
        command += str(e.key)
    if e.key == "Backspace":
        command = command[:-1] 
    if e.key == "Enter":
